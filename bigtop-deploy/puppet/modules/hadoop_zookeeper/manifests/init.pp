...
class hadoop_zookeeper (
  ...
  class common (
    ...
    bigtop_file::properties { "/etc/zookeeper/conf/log4j.properties":
      source => '/etc/zookeeper/conf/log4j.properties',
      overrides => $zookeeper_log4j_overrides,
  ...
  class server($myid,
    ...
    service { "zookeeper-server":
      ...
      subscribe => [ Bigtop_file::Properties["/etc/zookeeper/conf/zoo.cfg"],
                     File["/var/lib/zookeeper/myid"] ],
      hasrestart => true,
      hasstatus => true,
    }

    bigtop_file::properties { "/etc/zookeeper/conf/zoo.cfg":
      content => template("hadoop_zookeeper/zoo.cfg"),
      overrides => $zookeeper_config_overrides,
      require => Package["zookeeper-server"],
    }
