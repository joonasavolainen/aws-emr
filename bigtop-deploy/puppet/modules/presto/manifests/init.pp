...
class presto {
  ...
  class common($jvm_max_memory = ceiling(to_bytes($memorysize)*0.80), 
    ...
    #STDOUT Final Memory Decisions for Debuggin Purpouses. This could be helpful to support engineers 
    ...
    #Determine URI of Discovery Server
    $discovery_uri = "http://${discovery_host}:${discovery_port}"

    #Lets determine the Hive Metastore Details
    if ( hiera('hadoop_hive:common::hive_site_overrides[hive.metastore.uris]','undef') != "undef" ) {
      $hive_metastore_uri = hiera('hadoop_hive:common::hive_site_overrides[hive.metastore.uris]')
    } elsif hiera('hadoop_hive::common::metastore_server_host') {
      $hive_metastore_host = hiera('hadoop_hive::common::metastore_server_host')
      $hive_metastore_port = hiera('hadoop_hive::common::metastore_server_port')
  ...
  class manage_service {
    ...
    service { "presto-server":
      ...
      require => [ Package["presto"], Bigtop_file::Properties["/etc/presto/conf/config.properties"], Bigtop_file::Properties["/etc/presto/conf/catalog/hive.properties"] ],
      hasrestart => true,
      hasstatus => true,
      subscribe => [ Bigtop_file::Env["/etc/presto/conf/presto-env.sh"], Bigtop_file::Properties["/etc/presto/conf/catalog/hive.properties"], Bigtop_file::Properties["/etc/presto/conf/config.properties"], Bigtop_file::Properties["/etc/presto/conf/log.properties"], File['/etc/presto/conf/jvm.config'] ]
    }    
  }
